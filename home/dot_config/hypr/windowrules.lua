...
--
-- CORRECTED after you asked "do they also launch in workspace specified
...
-- real possibility per point 2, and something I can't test remotely), the
-- live-title rule is still there as a fallback for whatever a subsequent
-- open/focus/reflow does trigger. This is now my best-evidenced guess, not
-- a confirmed fix - genuinely un-testable without your hyprctl output.
--
-- IMPORTANT - every match pattern below is still an unverified guess, not
-- confirmed against your actual windows. After chezmoi apply, launch each
-- app FRESH (fully quit it first, not just refocus an already-open one) and
-- check both whether it landed on the right workspace AND what Hyprland saw:
--   hyprctl clients -j | jq '.[] | {class, title, workspace: .workspace.id}'
-- If something doesn't land right on first launch, that's the actual signal
-- to tell me which field was empty/wrong - I can't get closer than an
...
-- bindings.lua (^Bitwarden, ^[Ss]potify), not independently more/less
-- verified than those.
...
o.window("^[Bb]itwarden", { workspace = "3" }) -- live class
o.window({ initial_class = "^[Bb]itwarden" }, { workspace = "3" })
...
-- (whole title "X", or a "/ X" suffix) rather than a bare \bx\b. Watch this
-- one specifically after chezmoi apply - if it's grabbing windows it
-- shouldn't, tighten or drop this rule first.
o.window({ initial_title = "(?i)(^X$|/ X$)" }, { workspace = "4" })
o.window({ title = "(?i)(^X$|/ X$)" }, { workspace = "4" })
o.window("^[Ss]potify", { workspace = "4" }) -- live class
o.window({ initial_class = "^[Ss]potify" }, { workspace = "4" })
