...
`install-dev-stack.sh` installs and then keeps current: Java, Maven, Clojure CLI, Babashka, Polylith, Node/npm (LTS), Emacs + Doom Emacs (as a systemd --user daemon), the JetBrains Mono Nerd Font + Overpass fonts doom.d requires, Maple Mono Nerd Font, uv, curl, sqlite, tree, tre, jq, zathura, Citrix Workspace (icaclient), chezmoi, the Bitwarden CLI (`bw`) and Bitwarden Secrets Manager CLI (`bws`), and the GitHub CLI (`gh`).
...
- `mise use --global babashka@latest` — Babashka (`bb`), the fast-starting native-image Clojure scripting runtime. mise's own registry lists `babashka` as a first-class short name (backed by `github:babashka/babashka` release binaries, no separate plugin-add step needed) — same `step_mise()` code path as everything else in this list, just a one-line registry addition.
...
**Maple Mono Nerd Font — AUR, added on request as an extra terminal/Emacs font choice.** Unlike JetBrains Mono, Maple Mono bundles its Nerd Font icon glyphs as part of the font project itself rather than needing a separate "-nerd" patch package, so the family you get after installing is `Maple Mono NF` directly. Upstream's own install matrix (`font.subf.dev/en/download`) lists three plain-AUR package names depending on variant — `maplemono-ttf` (no icons), `maplemono-nf-unhinted` (icons, English-only — what's used here), and `maplemono-nf-cn-unhinted` (icons + CJK 2:1 width support) — plus separate, faster-installing equivalents (`ttf-maplemono*`) via the third-party ArchLinuxCN repo, which isn't used here since this setup doesn't add extra pacman repos anywhere else. This is a second font choice alongside JetBrains Mono Nerd Font, not a replacement — switch between them with `omarchy-font-set` or by hand in `doom.d/config.el`'s `doom-font`/`doom-big-font` (family `"Maple Mono NF"`). **Lower-confidence flag**, same class as the `bws-bin` note elsewhere in this doc: `aur.archlinux.org` itself is unreachable from this working environment (bot-walled), so the exact package name and its current maintenance status couldn't be double-checked directly against the AUR — it's sourced from the font's own official download page instead, which is a strong but not identical source. Confirm with `yay -Si maplemono-nf-unhinted` (or the script's own `./install-dev-stack.sh --check "Maple Mono"`) before relying on it, and see `CITRIX-WORKSPACE-MANUAL-INSTALL.md`'s pattern if the AUR build ever needs a manual step.
...
- [mise registry](https://mise.jdx.dev/registry.html) — confirms `babashka` is a documented short name (`github:babashka/babashka` backend, `mise use babashka@latest`), same as the other mise entries here
- [subframe7536/maple-font](https://github.com/subframe7536/maple-font) and its [download page](https://font.subf.dev/en/download/) — the font project's own README/site, source for the `maplemono-nf-unhinted` AUR package name and the `Maple Mono NF` family-name convention (see the "Lower-confidence flag" note above — `aur.archlinux.org` itself couldn't be reached directly to cross-check)
